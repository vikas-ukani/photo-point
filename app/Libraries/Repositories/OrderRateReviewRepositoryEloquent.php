...
class OrderRateReviewRepositoryEloquent extends BaseRepository implements UsersRepository
{
    ...
    protected function commonFilterFn(&$value, $input)
    {
        ...
        if (isset($input['search'])) {
            $value = $this->customSearch($value, $input, ['review']);
        }
        ...
        /** order_id and order_ids wise filter */
        if (isset($input['order_id'])) {
            $value = $value->where('order_id', $input['order_id']);
        }
        if (isset($input['order_ids']) && count($input['order_ids']) > 0) {
            $value = $value->whereIn('order_id', $input['order_ids']);
        }
        ...
        if (isset($input['rate'])) {
            $value = $value->where('rate', $input['rate']);
        }
    }
